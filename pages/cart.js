...
const CartScreen = () => {
  ...
  async function updateCartHandle(item, quantity) {
  ...
  return (<Layout title={"Shopping Cart"}>
    ...
    {cartItems.length === 0 ? (<div>Cart is empty. <NextLink href={"/"} passHref><Link>Go shopping</Link></NextLink></div>) : (
                ...
                {cartItems.map((item) => (<TableRow key={item._id}>
                  ...
                  <TableCell align={"right"}>
                    <Select value={item.quantity}
                            onChange={event => updateCartHandle(item, event.target.value)}>
